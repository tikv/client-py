...
impl Client {
    ...
    fn begin(&self, pessimistic: bool) -> PyCoroutine {
    ...
    fn current_timestamp(&self) -> PyCoroutine {
        let inner = self.inner.clone();
        PyCoroutine::new(async move {
    ...
    fn snapshot(&self, timestamp: u64) -> Snapshot {
        Snapshot {
            inner: Arc::new(
                self.inner
    ...
    fn gc(&self, safepoint: u64) -> PyCoroutine {
        let inner = self.inner.clone();
        PyCoroutine::new(async move {
            let result = inner
